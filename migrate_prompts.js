...
const db = new Database('./backend/agent_brain.db');
...
const PROMPT_DIR = path.join(__dirname, 'backend/prompts');
if (fs.existsSync(PROMPT_DIR)) {
  ...
  for (const f of files) {
    if (f.endsWith('.md')) {
      ...
      // Strip frontmatter if it exists standard regex
  ...
  console.log('No backend/prompts folder found!');
}
